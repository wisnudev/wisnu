...
async function setTemplateMenu(wisnu, type, m, prefix, setv, db, options = {}) {
...
╰──────❍`
	...
	if (type == 1 || type == 'buttonMessage') {
		await wisnu.sendButtonMsg(m.chat, {
		...
		let profile
		try {
			profile = await wisnu.profilePictureUrl(m.sender, 'image');
		} catch (e) {
			profile = fake.anonim
		}
		const menunya = `
